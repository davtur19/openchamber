...
`;
  ...
  describe('on Windows', () => {
    const tarballOf = (name) => `https://registry.npmjs.org/@opencode/${name}/-/${name}-2.0.16.tgz`;
    let exe;
    ...
    const runWindows = (archive, integrity = integrityOf(archive), arch = 'x64') => installOpenCodeV2({
      ...
      fetchImpl: async (url) => {
        ...
        if (url.endsWith(`/@opencode%2F${name}/2.0.16`)) return Response.json({ dist: { tarball: tarballOf(name), integrity } });
