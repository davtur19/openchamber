...
export const createSessionGoalRuntime = ({
  ...
  const tick = async (sessionId, directory) => {
    ...
    // messages) are retried across ticks instead of settling; the streak is
    // persisted in `turnErrorStreak` and only TURN_ERROR_RETRY_LIMIT (5)
    // consecutive transient failures block the goal. Permanent errors (auth,
    // not found, content policy, shape, and non-object error payloads) block
